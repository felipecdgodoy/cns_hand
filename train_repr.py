...
class Net(nn.Module):
    ...
    def __init__(self, image_dim=64, tau_dim=512):
        ...
        self.encode = nn.Sequential(
            ...
            nn.Sequential(nn.Conv3d(in_channels=16, out_channels=64, kernel_size=3, padding=1),
                          nn.ReLU(),
                          nn.MaxPool3d(kernel_size=2)),
            nn.Sequential(nn.Conv3d(in_channels=64, out_channels=64, kernel_size=3, padding=1),
                          nn.ReLU(),
                          nn.MaxPool3d(kernel_size=2)),
        ...
        self.linear = nn.Sequential(
            nn.Linear(in_features=tau_dim, out_features=(tau_dim // 2)),
            nn.Linear(in_features=(tau_dim // 2), out_features=(tau_dim // 4)),
            nn.Linear(in_features=(tau_dim // 4), out_features=(tau_dim // 8)),
            nn.Linear(in_features=(tau_dim // 8), out_features=(tau_dim // 16)),
            nn.Linear(in_features=(tau_dim // 16), out_features=(tau_dim // 32)),
            nn.Linear(in_features=(tau_dim // 32), out_features=(tau_dim // 64)),
            nn.Linear(in_features=(tau_dim // 64), out_features=2),
            nn.Sigmoid()
        )
    ...
    def train_fold(self, image_pairs, label_pairs, sigma_pairs, delta_sigma_pairs, model_args, fold, verbose=1):
        ...
        print(f'[DONE] FOLD {fold} --- Best Epoch Results -> TRAIN ACC = {round(100*train_acc[best_index], 2)}% | TEST ACC = {round(100*test_acc[best_index], 2)}%\n')
...
if __name__ == '__main__':
    ...
    dir_destination = f'epochs = {epochs} - alpha = {alpha} - batch_size = {batch_size} - lambda = {lamb}'
    try:
